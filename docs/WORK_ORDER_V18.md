# Sumcheck v1.8.0 — Language Cycle Work Order (Spanish, Portuguese, French)
...
Bump both manifests to **1.8.0-dev** before any other change. Lesson from the last cycle: the packaging gate rebuilds `dist/sumcheck-<version>.zip` on every run, which silently overwrote the 1.5.0 artifact. With the bump first, every mid-cycle package lands in `sumcheck-1.8.0-dev.zip` and `sumcheck-1.6.0.zip` stays pristine for its eventual upload. The final task renames to 1.8.0 proper. If `-dev` in the version string trips any gate assertion, fix the assertion, not the version.
...
- Version → **1.8.0** (drop `-dev`) in both manifests. CHANGELOG entry.
- Full gates: `npm run check`, `npm test`, `verify-extension` against the packaged 1.8.0 zip, full corpus re-score, marker invariant, lexicon 50/50 on English.
- `dist/sumcheck-1.8.0.zip` built. Confirm `dist/sumcheck-1.6.0.zip` is byte-identical to its state at cycle start (record its SHA-256 in the L0 note and re-verify here).
